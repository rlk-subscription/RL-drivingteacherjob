...
local function SpawnAirplane(licence)
    ...
    if not QBCore.Functions.SpawnClear(tmpSpawnPosition, 5.0) then
        ...
        QBCore.Functions.SpawnVehicle(Config.VehicleModels[licence], function(_vehicle)
            ...
            TriggerEvent('RL-drivingteacherjob:client:giveKeys', plate, _vehicle)
        ...
        if PlayerData.job.name == 'drivingteacher' then
            ...
            exports['qb-target']:AddBoxZone("drivingteacher_"..k, v, 1, 1, {
                ...
                options = {
                    {
                        type = "client",
                        event = "RL-drivingteacherjob:client:ToggleDuty",
...
RegisterNetEvent('RL-drivingteacherjob:client:giveKeys', function(plate, vehicle)
    TriggerServerEvent(Config.GiveKeyScriptTrigger, plate)
    if Config.UseMHVehicleKeyItem then exports['RL-vehiclekeyitem']:CreateTempKey(vehicle) end
end)

RegisterNetEvent('RL-drivingteacherjob:client:removeKeys', function(plate)
    TriggerEvent(Config.RemoveKeyScriptTrigger, plate)		
end)

RegisterNetEvent('RL-drivingteacherjob:client:giveLicinceMenu', function()
    local playerlist = {}
    QBCore.Functions.TriggerCallback('RL-drivingteacherjob:server:GetOnlinePlayers', function(online)
        ...
        if menu then
            if not menu.id and not menu.licence then
                ...
                TriggerServerEvent('RL-drivingteacherjob:server:givelicince', tonumber(menu.id), tostring(menu.licence))
            end
        end
    end)
end)

RegisterNetEvent('RL-drivingteacherjob:client:takeLicinceMenu', function()
    local playerlist = {}
    QBCore.Functions.TriggerCallback('RL-drivingteacherjob:server:GetOnlinePlayers', function(online)
        for key, v in pairs(online) do
            playerlist[#playerlist + 1] = {value = v.source, text = "(ID:"..v.source..") "..v.fullname}
        ...
        if menu then
            if not menu.id then
                ...
                TriggerServerEvent('RL-drivingteacherjob:server:takelicince', tonumber(menu.id))
            end
        end
    end)
end)

RegisterNetEvent('RL-drivingteacherjob:client:vehicleMenu', function()
    GetNeerestGarage()
...
RegisterNetEvent('RL-drivingteacherjob:client:ToggleDuty', function()
                ...
                if PlayerData.job.name == 'drivingteacher' and PlayerData.job.onduty then
                    if IsPedInAnyVehicle(ped, false) then
                        ...
                        if plate == currentPlate then
                            ...
                            Wait(1500)
                            if Config.UseMHVehicleKeyItem then exports['RL-vehiclekeyitem']:DeleteKey(QBCore.Functions.GetPlate(veh)) end
                            QBCore.Functions.DeleteVehicle(veh)
                        else
                            QBCore.Functions.Notify(Lang:t('notify.wrong_vehicle_to_park'))
                        end
                    else
                        TriggerEvent('RL-drivingteacherjob:client:vehicleMenu', pos)
                    end
                end
            end
        end
