...
QBCore.Functions.CreateCallback("RL-drivingteacherjob:server:GetOnlinePlayers", function(source, cb)
	cb(GetOnlinePlayers())
end)
...
RegisterServerEvent('RL-drivingteacherjob:server:givelicince', function(id, license)
	local src = source
...
RegisterServerEvent('RL-drivingteacherjob:server:takelicince', function(id)
	local src = source
	local Player = QBCore.Functions.GetPlayer(src)
	if id and tonumber(id) >= 1 then
